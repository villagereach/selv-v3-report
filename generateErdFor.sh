...
&& mkdir output \
&& chmod 777 output \
&& (docker run --rm --network openlmisreporterdgeneration_default -v $WORKSPACE/output:/output schemaspy/schemaspy:snapshot -t pgsql -host db -port 5432 -db open_lmis -s report -u postgres -p $DBPASSWORD -I "(data_loaded)|(schema_version)|(jv_.*)" -norows -hq &) \
&& sleep 30 \
...
&& zip -r $WORKSPACE/erd-report.zip . \
&& popd \
&& rmdir output \
&& rm .env \
